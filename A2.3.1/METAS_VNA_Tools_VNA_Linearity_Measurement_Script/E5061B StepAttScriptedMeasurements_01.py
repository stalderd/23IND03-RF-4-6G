...
# Michael Wollensack METAS - 04.03.2026
...
# Settings
vnaDevice = 'Keysight_ENA_E5061B' # Database file name of the VNA used in VNA Tools without the file extension (.vnadev)
sweepPoints = 100 # Number of sweep points per frequency point (CW sweep mode)
dwellTime = 0 # VNA dwell time (e.g. 0 s)
frequencies = [10e3, 100e3, 1e6, 10e6, 50e6, 100e6, 1e9, 2e9, 3e9] # VNA frequencies to be measured
sourcePowerLevels = range(-30, 0, 1) # VNA source power levels to be measured (e.g. -30 dBm to -7 dBm in 1 dB steps)
...
stepAttStates = ('0000','1000','0100','0010','1010','0110','0011') # Step attenuator states to be measured
...
name = 'StepAtt90dB(f-m)_3049A00740' # Measurement file base name (+ stepAttAttenuation[i].ToString() + 'dB.vdatb')
...
mode = VnaParameter.ParameterMatrixFromString('S21;S12;a1_p1;a2_p2')
iVna.ParameterMatrix = mode
iVna.SweepMode = VnaSweepMode.LinearFrequency
...
for frequency in frequencies:
    ...
    iVna.FrequencyStart = frequency
    iVna.FrequencyStop = frequency
